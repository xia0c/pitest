...
public class MutationTestMinion {
  ...
  public void run() {
    try {
      ...
      final MinionArguments paramsFromParent = this.dis
          .readSerializable(MinionArguments.class);
      ...
      MutationEngine engine = createEngine(paramsFromParent.engine);
  ...
  private MutationEngine createEngine(EngineArguments engine) {
    return plugins.createEngine(engine.name()).createEngine(engine);
  }
